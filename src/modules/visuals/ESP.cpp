...
static void RenderBoxESP(const cache::entity_t& entity, const math::Matrix4x4& view) {
	if (entity.humanoid.address != cache::cachedLocalPlayer.humanoid.address && entity.humanoid.address) {
		...
		if (math::math::WorldToScreen(entity.position, footPosition2D, view) && math::math::WorldToScreen(entity.headPosition, headposition2D, view) && entity.humanoid.address) {
			ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(footPosition2D.x - ((headposition2D.y - footPosition2D.y) * 0.225f), headposition2D.y), ImVec2(footPosition2D.x + ((headposition2D.y - footPosition2D.y) * 0.225f), footPosition2D.y), IM_COL32(255, 0, 0, 200));
		}
	}
}
